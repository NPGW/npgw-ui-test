...
public class FraudControlTest extends BaseTest {
    ...
    @TmsLink("1143")
    @Epic("System/Fraud control")
    @Feature("'Change control activity' dialog text content")
    ...
    public void testVerifyWarningModalWindowChangeActivityForControlTable() {
        ...
        assertThat(dialog.getDialogHeader()).hasText("Change control activity");

        Allure.step("Verify Deactivate control modal window main body text");
        assertThat(dialog.getModalWindowsMainTextBody()).hasText("Are you sure you want to deactivate control "
                + FRAUD_CONTROL.getControlDisplayName() + "?");

        ActivateControlDialog activateDialog = dialog.clickCancelButton()
                .getTableControls().clickActivateControlButton(FRAUD_CONTROL_INACTIVE.getControlName());

        Allure.step("Verify Activate control modal window text");
        assertThat(activateDialog.getDialogHeader()).hasText("Change control activity");

        Allure.step("Verify Activate control modal window main body text");
        assertThat(activateDialog.getModalWindowsMainTextBody()).hasText("Are you sure you want to activate control "
